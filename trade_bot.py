...
def send_trade_signal(ticker, action, price, amount, reason, strength="약함"):
    ...
    try:
        signal_map = {
            "매수": "buy", "반매수": "half_buy", "반매도": "half_sell",
            "매도": "sell", "손절": "sell"
        }
        ...
        payload = {
            "token": SECRET_TOKEN, "ticker": ticker, "signal": signal_type,
            "action": action, "price": float(price), "amount": amount, "reason": reason
        }
        ...
        if response.status_code == 200:
            if action in ["매수", "반매수"]:
                ...
                if action == "매수":
                    ...
                    half_buy_count += 1
                    if first_half_buy_price == 0.0: first_half_buy_price = price
                    last_half_buy_time = time.time()
                    logger.info("🔄 반매수로 인해 반매도 횟수가 0으로 초기화되었습니다.")
            elif action in ["매도", "손절"]:
                position_open, entry_price, total_held_volume_btc, total_bought_amount_krw = False, 0.0, 0.0, 0.0
                strong_buy_count, half_buy_count, half_sell_count, first_half_buy_price = 0, 0, 0, 0.0
                last_buy_time, last_half_buy_time, last_half_sell_time = None, None, None
                ...
                if total_held_volume_btc > sold_volume_btc:
                    ...
                    logger.warning(f"⚠️ 보유 수량보다 많은 반매도 시도. 전량 매도로 처리합니다.")
                    position_open, entry_price, total_held_volume_btc, total_bought_amount_krw = False, 0.0, 0.0, 0.0
                    strong_buy_count, half_buy_count, first_half_buy_price, half_sell_count = 0, 0, 0.0, 0
                ...
                if position_open:
                    strong_buy_count, half_buy_count, first_half_buy_price = 0, 0, 0.0
...
def ask_gpt(ticker, indicators):
    ...
    **매매 기회를 포착하는 데 있어서 과도한 보수성보다는, 합리적인 위험을 감수하고 잠재적인 기회를 적극적으로 탐색하는 데 중점을 두세요.**
    ...
    - 매도:
        - 높음:** 다음 조건들 중 **세 가지 이상**이 매우 명확하게 나타날 경우, 시장의 강력한 하락 전환 가능성을 시사하므로 '매도' 시그널과 함께 **반드시 '강함' 신뢰도**를 반환해야 합니다.
            ...
            - 가격 고점 형성 후 하락 반전, **최근 상승봉 개수 감소 추세 또는 10봉 변화율이 음수로 전환**은 강한 매도 신호입니다.
...
while True:
    try:
        ...
        df['long_term_down'] = (df['ma_60'] < df['ma_120']) & (df['ma_120'] < df['ma_240'])
        df['trend_down_filter'] = (df['ma_20'] < df['ma_60']) & df['long_term_down'] & (df['adx'] >= ADX_THRESHOLD)

        recent_df_for_fib = df[-100:].copy()
        high = recent_df_for_fib['high'].max()
        low = recent_df_for_fib['low'].min()
        price_diff = high - low
        fib_0, fib_382, fib_500, fib_618 = high, high - price_diff * 0.382, high - price_diff * 0.5, high - price_diff * 0.618
        fib_786, fib_886, fib_100 = high - price_diff * 0.786, high - price_diff * 0.886, low

        hidden_bullish, hidden_bearish, regular_bullish, regular_bearish = detect_hidden_and_regular_divergence(df)
        latest = df.iloc[-1]
        
        indicators = {
            "close": latest.get('close', 0), "macd": latest.get('macd', 0), "macd_signal": latest.get('macd_signal', 0),
            "macd_cross": latest.get('macd_cross', False), "rsi": latest.get('rsi', 50), "di": latest.get('di', 0),
            "ma_cross": latest.get('ma_cross', False), "volume_change": latest.get('volume_change', 0),
            "bb_upper": latest.get('bb_upper', 0), "bb_lower": latest.get('bb_lower', 0), "volatility": latest.get('volatility', 0),
            "rate_3": latest.get('rate_3', 0), "rate_10": latest.get('rate_10', 0), "oversold": latest.get('oversold', False),
            "overbought": latest.get('overbought', False), "fib_0": fib_0, "fib_382": fib_382, "fib_500": fib_500,
            "fib_618": fib_618, "fib_786": fib_786, "fib_886": fib_886, "fib_100": fib_100,
            "hidden_bullish": hidden_bullish, "hidden_bearish": hidden_bearish,
            "regular_bullish": regular_bullish, "regular_bearish": regular_bearish,
            "up_candles": latest.get('up_candles', 0), "above_5ma": latest.get('above_5ma', False),
            "adx": latest.get('adx', 0), "trend_down_filter": latest.get('trend_down_filter', False),
            "long_term_down": latest.get('long_term_down', False)
        }
        ...
        if gpt_decision == "매수" and indicators.get('trend_down_filter', False):
            logger.info("🚫 하락 추세 필터에 의해 매수 시그널 무시")
            gpt_decision = "관망"
            gpt_strength = "약함"
            gpt_reason += " (하락 추세)"
        ...
        final_strength = "약함"
        if gpt_decision == "매수":
            if indicators.get('volatility', 0) > VOLATILITY_THRESHOLD: strength_score += 1
            if indicators.get('rsi', 50) < RSI_OVERSOLD: strength_score += 1
            if indicators.get('rate_3', 0) > 0.005: strength_score += 1
            if indicators.get('macd', 0) > indicators.get('macd_signal', 0): strength_score += 2
            if indicators.get('macd_cross', False): strength_score += 1
            if indicators.get('hidden_bullish', False): strength_score += 1
            if indicators.get('regular_bullish', False): strength_score += 1
            if indicators.get('volume_change', 0) > 0: strength_score += 1
            if indicators.get('rate_10', 0) > 0: strength_score += 1
            if indicators.get('above_5ma', False): strength_score += 1
            if indicators.get('trend_down_filter', False) or indicators.get('rate_10',0) < 0:
                strength_score -= 2
            if strength_score >= 7: final_strength = "강함"
            elif 4 <= strength_score < 7: final_strength = "중간"
        elif gpt_decision == "매도":
            if indicators.get('volatility', 0) > VOLATILITY_THRESHOLD: strength_score += 1
            if indicators.get('rsi', 50) > RSI_OVERBOUGHT: strength_score += 1
            if indicators.get('rate_3', 0) < -0.003: strength_score += 1
            if indicators.get('macd', 0) < indicators.get('macd_signal', 0): strength_score += 1
            if indicators.get('hidden_bearish', False): strength_score += 1
            if indicators.get('regular_bearish', False): strength_score += 1
            if not indicators.get('above_5ma', False): strength_score += 1
            if gpt_strength == "강함": strength_score += 1
            if strength_score >= 5: final_strength = "강함"
            elif 4 <= strength_score < 5: final_strength = "중간"

        current_price = indicators["close"]
        logger.info(f"📊 현재 상태: PosOpen={position_open}, Entry={entry_price:,.0f}, "
        ...
        action_taken, action_reason, trade_amount = None, None, 0
        
        if position_open:
            profit = (current_price - entry_price) / entry_price if entry_price != 0 else 0
            if profit <= STOP_LOSS:
                action_taken, action_reason = "손절", f"손절 조건 충족 (손실율: {profit:.2%})"
                trade_amount = total_held_volume_btc
            elif profit >= TAKE_PROFIT:
                action_taken, action_reason = "매도", f"익절 조건 충족 (수익율: {profit:.2%})"
                trade_amount = total_held_volume_btc
            elif gpt_decision == "매도":
                lower_bound, upper_bound = entry_price * 0.999, entry_price * 1.001
                if lower_bound <= current_price <= upper_bound:
                    logger.info(f"🚫 현재가({current_price:,.0f})가 평단가({entry_price:,.0f})의 +-0.1% 이내이므로 매도/반매도 무시.")
                elif final_strength == "강함":
                    action_taken, action_reason = "매도", f"강함 매도: {gpt_reason}"
                    trade_amount = total_held_volume_btc
                elif final_strength == "중간" and half_sell_count < 5:
                    if last_half_sell_time is None or (time.time() - last_half_sell_time) >= TRADE_COOL_DOWN_SECONDS:
                        action_taken, action_reason = "반매도", f"중간 매도 (반매도 {half_sell_count + 1}회차): {gpt_reason}"
                        trade_amount = round(total_held_volume_btc / 2, 8)
                        if trade_amount * current_price < 5000: trade_amount = total_held_volume_btc
                    else:
                        logger.info(f"⏳ 반매도 쿨다운. 스킵.")
        
        if action_taken is None and gpt_decision == "매수" and not indicators.get('trend_down_filter', False):
            if todays_open_price is not None and current_price <= (todays_open_price * 0.99):
                logger.info(f"🚫 현재가({current_price:,.0f})가 당일 시가({todays_open_price:,.0f}) 대비 -1% 이하이므로 매수 금지.")
            else:
                if last_strong_sell_time is not None and (time.time() - last_strong_sell_time) < TRADE_COOL_DOWN_SECONDS:
                    remaining_cooldown = int(TRADE_COOL_DOWN_SECONDS - (time.time() - last_strong_sell_time))
                    logger.info(f"🚫 강한 매도 후 {remaining_cooldown}초 남음. 매수 금지.")
                else:
                    if final_strength == "강함" and strong_buy_count < 10:
                        if last_buy_time is None or (time.time() - last_buy_time) >= TRADE_COOL_DOWN_SECONDS:
                            if not position_open or current_price < entry_price * (1 + 0.003):
                                action_taken, trade_amount = "매수", TRADE_BUY_AMOUNT_KRW_STRONG
                                action_reason = f"강함 매수 ({strong_buy_count + 1}회차): {gpt_reason}"
                            else:
                                logger.info(f"🚫 강함 매수 조건 불충족: 현재가가 평단가보다 0.3% 이상 높음.")
                        else:
                            logger.info(f"⏳ 강함 매수 쿨다운. 스킵.")
                    elif final_strength == "중간":
                        if last_half_buy_time is None or (time.time() - last_half_buy_time) >= TRADE_COOL_DOWN_SECONDS:
                            if half_buy_count < 15:
                                if not position_open or (entry_price > 0 and current_price <= entry_price * (1 + 0.005)):
                                    action_taken, trade_amount = "반매수", TRADE_BUY_AMOUNT_KRW_HALF
                                    action_reason = f"중간 매수 (반매수, {half_buy_count + 1}회차): {gpt_reason}"
                                else:
                                    logger.info(f"🚫 반매수 조건 불충족: 현재가가 평단가보다 0.5% 이상 높음.")
                            else:
                                logger.info(f"🚫 반매수 횟수 제한(15회) 도달.")
                        else:
                            logger.info(f"⏳ 반매수 쿨다운. 스킵.")
        
        final_decision_text, final_reason_text = "관망", gpt_reason
        log_strength = final_strength

        if action_taken:
            final_decision_text, final_reason_text = action_taken, action_reason
            signal_sent_success = send_trade_signal(TICKER, action_taken, current_price, trade_amount, action_reason, final_strength)
            signal_status_text = "📡 시그널 전송 성공" if signal_sent_success else "❌ 시그널 전송 실패"
            final_log_message = (
            ...
            final_reason_text = f"보유: 매매 조건 불충족 (GPT 판단: {gpt_decision}/{gpt_strength}, 사유: {gpt_reason})"
            if action_reason: final_reason_text = f"보유: {action_reason}"
            final_log_message = f"📊 최종 결정: {final_decision_text} | 신뢰도: {log_strength} | 사유: {final_reason_text}"
        else:
            final_decision_text = "관망"
            final_reason_text = f"관망: 매수/매도 조건 불충족 (GPT 판단: {gpt_decision}/{gpt_strength}, 사유: {gpt_reason})"
            final_log_message = f"📊 최종 결정: {final_decision_text} | 신뢰도: {log_strength} | 사유: {final_reason_text}"

        logger.info(final_log_message)
        send_telegram_message(gpt_decision, gpt_strength, gpt_reason, final_decision_text, log_strength, final_reason_text)
